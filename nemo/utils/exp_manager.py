...
class EarlyStoppingParams:
    ...
    patience: int = 10
    min_epochs: int = (
        0  # minimum number of epochs to run before enabling early stopping
    )
    verbose: bool = True
    strict: bool = True
...
class CustomEarlyStopping(EarlyStopping):
    ...
    def _run_early_stopping_check(self, trainer) -> None:
        """Overrides the early stopping check to include a minimum epoch requirement."""
...
def exp_manager(
    ...
    if cfg.create_early_stopping_callback:
        early_stop_callback = CustomEarlyStopping(**cfg.early_stopping_callback_params)
        trainer.callbacks.append(early_stop_callback)

    if cfg.create_ipl_epoch_stopper_callback:
        ipl_epoch_stopper_callback = IPLEpochStopper(
            **cfg.ipl_epoch_stopper_callback_params
        )
